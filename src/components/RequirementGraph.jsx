...
function buildGraph(requirements, program, onOpen, selectedIncome) {
  ...
  return { nodes: graphNodes, edges, focusNode };
}

function FocusNextStep({ node }) {
  ...
  useEffect(() => {
    ...
    const centerNextStep = () => {
      ...
      timer = window.setTimeout(() => {
        ...
        setCenter(node.position.x + NODE_WIDTH / 2, node.position.y + NODE_HEIGHT / 2 + 132, { zoom: 0.92, duration: 420 });
      }, 80);
    };
    centerNextStep();
  ...
  return (
    <section className="requirement-graph-panel" aria-labelledby="requirement-graph-title">
      <div className="graph-panel-heading">
        ...
        <span className="graph-focus-badge"><Sparkles />Centered on your next step</span>
      </div>
      <p className="panel-intro">Follow the arrows to see what unlocks next. Drag the canvas to explore or use the visible zoom controls.</p>
      <div className="requirement-graph-canvas" aria-label="Interactive graph of document requirements">
        ...
        >
          <FocusNextStep node={graph.focusNode} />
